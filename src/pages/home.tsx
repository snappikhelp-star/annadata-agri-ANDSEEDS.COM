        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 md:gap-4 max-w-4xl mx-auto">
          {problems.map((p, i) => (
            <motion.button key={i} onClick={() => setSelected(p.label)}
              ...
              className="group relative rounded-2xl p-4 md:p-5 text-center border-2 bg-white transition-all cursor-pointer"
              style={{ borderColor: `${p.color}40`, boxShadow: `0 4px 20px ${p.color}15` }}
            >
...
function CropDoctorSection() {
  ...
  const problems = [
    ...
    { emoji: "🐛", label: "कीड़े लग रहे है", tip: "रस चूसक कीट (BPH) या इल्ली हो सकती है। सही कीटनाशक की Spray जरूरी है — Keshav Bhai से पूछें।" },
    ...
    <section className="py-14 md:py-20 bg-foreground relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 md:px-6 relative z-10">
        <div className="text-center mb-10">
          ...
          <p className="text-white/60 font-hindi mt-2">समस्या चुनें — तुरंत सलाह पाएं और Keshav Bhai से WhatsApp पर बात करें</p>
          <motion.div initial={{ scaleX: 0 }} whileInView={{ scaleX: 1 }} viewport={{ once: true }} transition={{ delay: 0.2, duration: 0.8 }}
            className="w-24 h-1 bg-secondary mx-auto mt-4 rounded-full" />
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 md:gap-4 max-w-4xl mx-auto mb-8">
          {problems.map((p, i) => (
            <motion.button key={i} onClick={() => setSelected(selected === i ? null : i)}
              initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
              transition={{ delay: i * 0.05 }} whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.97 }}
              className={`flex flex-col items-center gap-2 p-4 rounded-2xl border-2 text-center transition-all cursor-pointer font-hindi text-sm font-semibold ${
                selected === i
                  ? "bg-secondary text-foreground border-secondary shadow-xl"
                  : "bg-white/5 text-white border-white/10 hover:border-secondary/60 hover:bg-white/10"
              }`}>
        ...
        <AnimatePresence>
          {selected !== null && (
            <motion.div key={selected} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}
              className="max-w-2xl mx-auto bg-white/10 backdrop-blur-sm border border-secondary/40 rounded-3xl p-6 text-center">
              <div className="text-4xl mb-3">{problems[selected].emoji}</div>
              <h3 className="text-white font-hindi font-black text-xl mb-3">{problems[selected].label}</h3>
              <p className="text-white/80 font-hindi text-base mb-5 leading-relaxed bg-white/5 p-4 rounded-2xl">
                💡 {problems[selected].tip}
              </p>
              <a href={waLink(`नमस्ते Keshav Bhai! मेरी फसल में समस्या है: *${problems[selected].label}*\nकृपया सलाह दें।`)}
                target="_blank" rel="noreferrer"
                className="inline-flex items-center gap-2 px-8 py-3.5 bg-[#25D366] text-white font-hindi font-bold rounded-2xl hover:bg-[#1ebe5d] transition-all hover:scale-105 shadow-xl text-base">
                <FaWhatsapp className="w-5 h-5" /> Keshav Bhai से अभी पूछें
              </a>
            </motion.div>
          )}
        </AnimatePresence>
          ...
          {products.map((product, i) => (
            <div key={i}>
              <motion.div
                ...
                className={`bg-gradient-to-br ${product.bg} rounded-2xl p-5 md:p-6 relative overflow-hidden group cursor-pointer transition-all`}
                style={{ boxShadow: activeProduct === i ? `0 0 0 3px ${product.accent}, 0 16px 40px rgba(0,0,0,0.4)` : `0 12px 40px -8px rgba(0,0,0,0.4), inset 0 0 0 1px rgba(255,255,255,0.08)` }}
              >
      ...
      <div className="container mx-auto px-4 md:px-6">
        <div className="text-center mb-8 md:mb-12">
          ...
          <motion.p initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ delay: 0.2 }}
            ...
            महीना चुनें और सही सलाह पाएं — Keshav Bhai से
          </motion.p>
          <motion.div initial={{ scaleX: 0 }} whileInView={{ scaleX: 1 }} viewport={{ once: true }} transition={{ delay: 0.25, duration: 0.8 }}
            className="w-24 h-1 bg-secondary mx-auto mt-4 rounded-full" />
        </div>
        ...
        {/* Month Tabs */}
        <div className="flex gap-2 overflow-x-auto pb-3 mb-6 scrollbar-hide justify-start md:justify-center">
          {months.map((m, i) => (
        ...
        <AnimatePresence mode="wait">
          <motion.div key={activeTab} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -16 }}
            ...
            {months[activeTab].items.map((item, j) => (
              <motion.div key={j} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: j * 0.07 }}
                className="bg-white rounded-2xl p-5 border border-border shadow-sm hover:shadow-md transition-all group">
                ...
                <h4 className="font-hindi font-bold text-foreground text-base mb-1.5 leading-tight">{item.label}</h4>
                <p className="text-muted-foreground text-xs font-hindi leading-relaxed mb-4">{item.tip}</p>
                <a href={waLink(`🌾 *Annadata Agri — फसल कैलेंडर Enquiry*\n\n📅 महीना: ${months[activeTab].name}\n📌 विषय: ${item.label}\n\nKeshav Bhai, इस बारे में सलाह चाहिए।`)}
                  target="_blank" rel="noreferrer"
                  className="inline-flex items-center gap-1.5 text-xs font-bold text-white bg-[#25D366] px-3 py-2 rounded-full hover:bg-[#1ebe5d] transition-colors font-hindi">
...
function DhanUthwaiSection() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const msg = `🚜 *धान घर से उठवाना है Request — अन्नदाता*\n\n👤 नाम: ${form.name}\n📱 मोबाइल: ${form.mobile}\n🏡 गांव: ${form.village}\n⚖️ अनुमानित Quantity: ${form.qty}\n🗓️ कटाई कब होगी: ${form.harvest}\n\nकृपया उठवाना की व्यवस्था करें।`;
    window.open(waLink(msg), "_blank");
    setSent(true);
    setTimeout(() => setSent(false), 4000);
  };
      ...
      <div className="container mx-auto px-4 md:px-6 max-w-3xl relative z-10">
        ...
        <div className="text-center mb-6">
          ...
          <div className="flex items-center justify-center gap-3 mb-3">
            <span className="flex items-center gap-1.5 bg-red-500/20 text-red-400 font-bold px-3 py-1 rounded-full text-xs border border-red-500/30">
              <span className="w-2 h-2 rounded-full bg-red-400 animate-pulse inline-block" />
              LIVE Booking
            </span>
            <span className="inline-flex items-center gap-1.5 bg-secondary/20 text-secondary font-bold px-3 py-1 rounded-full text-xs font-hindi border border-secondary/30">
              🏠 Online Order
            </span>
            <span className="inline-flex items-center gap-1.5 bg-green-500/20 text-green-300 font-bold px-3 py-1 rounded-full text-xs font-hindi border border-green-500/30">
              🌱 खरीफ 2026
            </span>
          </div>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
          ...
          <div className="bg-white/5 border border-green-400/40 rounded-2xl p-4 text-center relative overflow-hidden"
          ...
          <div className="bg-white/5 border border-yellow-400/40 rounded-2xl p-4 text-center relative overflow-hidden"
        ...
        <div className="grid grid-cols-3 sm:grid-cols-6 gap-2 mb-6">
          {features.map((f, i) => (
            <motion.div key={i} initial={{ opacity: 0, scale: 0.85 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }}
              ...
              className="flex flex-col items-center gap-1.5 bg-secondary/10 border border-secondary/25 rounded-2xl py-3 px-2 text-center">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 md:gap-7">
          {reasons.map((r, i) => (
            <motion.a key={i} href={waLink(`नमस्ते Keshav Bhai! मैं अन्नदाता की खासियत "${r.title}" के बारे में अधिक जानना चाहता हूँ।`)}
              target="_blank" rel="noreferrer"
              initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
              transition={{ delay: i * 0.08, duration: 0.5 }} whileHover={{ y: -8, scale: 1.03 }} whileTap={{ scale: 0.97 }}
...
function SecondTickerStrip() {
  const items = [
    ...
    "🚜 घर से धान उठवाना है — अन्नदाता की खास सेवा",
        ...
        <div className="border-t border-white/10 pt-6 flex flex-col items-center gap-2 text-center">
          ...
          <p className="text-background/40 text-xs">
            ...
            <a href="https://www.instagram.com/priyamxmedia" target="_blank" rel="noreferrer"
              className="font-black tracking-widest text-secondary/80 hover:text-secondary transition-all duration-300 hover:drop-shadow-[0_0_8px_rgba(249,168,37,0.8)] hover:tracking-[0.2em]"
